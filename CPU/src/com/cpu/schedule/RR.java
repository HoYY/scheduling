...
public class RR implements Algorism {
	...
	public Map<String, LinkedList<Process>> insertBlocked(Map<String, LinkedList<Process>> State) {
		if(!State.get("running").isEmpty()) {
			if(State.get("running").get(0).getRcpu_burst_time() == 0) {
				...
				State.get("running").get(0).setQ(0);
	...
	public Map<String, LinkedList<Process>> useCPU(Map<String, LinkedList<Process>> State) {
		if(!State.get("running").isEmpty()) {
			...
			State.get("running").get(0).setQ(State.get("running").get(0).getQ()+1);
		}
		return State;
	}
	...
	public Map<String, LinkedList<Process>> QuantumCheck(Map<String, LinkedList<Process>> State, int q){
		if(!State.get("running").isEmpty()) {
			if(State.get("running").get(0).getQ() == q) {
				State.get("running").get(0).setQ(0);
